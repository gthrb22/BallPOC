...
class GameScene: SKScene {
    ...
    override func didMoveToView(view: SKView) {
        addOverlay("Level 1",score: "Score:0")
  
    }
    func addOverlay (completionLevel:String,score:String) {
        overLayView = OverlayView.init(frame: CGRectMake(0, 0, (self.view?.frame.size.width)!, 50))
        overLayView!.levelCompletionlbl?.text = completionLevel
        overLayView!.scorelbl?.text = score
    ...
    func addTapToHoldBalls(ballstoAdd:Int){
        ...
        for i in 0..<ballstoAdd {
            ...
            shape.physicsBody!.applyImpulse(CGVectorMake(5, -5))
    ...
    func animateTextChange (text:String) {
        ...
        overLayView?.levelCompletionlbl?.layer.addAnimation(animation, forKey: "kCATransitionFade")
        overLayView?.levelCompletionlbl?.text = text
    }
    func animateScoreChange(text:String){
        let animation = CATransition()
        animation.timingFunction = CAMediaTimingFunction.init(name: kCAMediaTimingFunctionEaseInEaseOut)
        animation.type = kCATransitionFade
        animation.duration = 0.75
        overLayView?.scorelbl?.layer.addAnimation(animation, forKey: "kCATransitionFade")
        overLayView?.scorelbl?.text = "Score:\(text)"
    }
    override func update(currentTime: CFTimeInterval) {
        //1
        print(freeBallDestroyed)
        
        animateScoreChange("\(freeBallDestroyed)")
        ...
        if !gameOver {
            ...
            case 0:
                if (freeBallDestroyed>1 && freeBallDestroyed == 30) {
                    overLayView?.levelCompletionlbl?.text = "Level 1 Completed :)"
                    animateTextChange("Level 2")
            ...
            case 1 :
                if (freeBallDestroyed>1 && freeBallDestroyed == 30) {
                    overLayView?.levelCompletionlbl?.text = "Level 2 Completed :)"
                    animateTextChange("Level 3")
            ...
            case 2 :
                if (freeBallDestroyed>1 && freeBallDestroyed == 30) {
                    overLayView?.levelCompletionlbl?.text = "Level 3 Completed :)"
                    animateTextChange("Level 4")
    ...
    func addFreeBalls(){
        ...
        shape.runAction(
            SKAction.moveByX(-size.width - shape.frame.size.width, y: 10.0,
